...
<%     
        ...
        Connection myCon = DriverManager.getConnection("jdbc:mysql://localhost:3306/test1","root","password");
            ...
            //String query = "select courseid from studenttakes where studentid=?;";
            String query = "select courseid from studenttakes where NOT EXISTS (Select courseid from gradecenter where studenttakes.courseid=gradecenter.courseid AND studentid=?)";
...
<% if(!myRes.next()) {
    out.println("Sorry, could not find any assignments for you");
}else{%>
<h3>Assignement</h3>
</hr/>
...
<%
	 myRes.first();%>
	  <tr><a href="StudentAssignmentServlet?courseid=<%=myRes.getString("courseid")%>&action=actionsend1"><%=myRes.getString("courseid")%></a></tr> 
	 <br/>
